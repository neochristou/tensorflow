...
class RiscBinaryComparisonOp : public OpKernel {
 public:
  ...
  void do_RiscBinaryComparisonOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscBinaryComparison op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscBinaryComparisonOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscBinaryComparisonOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscBinaryComparisonOp(fuzz_ctx);
          fuzzer.mut_end_time(fuzz_ctx);
        }

        tffuzzing::already_fuzzing = false;
        do_RiscBinaryComparisonOp(ctx);
      } else {
        do_RiscBinaryComparisonOp(ctx);
      }
