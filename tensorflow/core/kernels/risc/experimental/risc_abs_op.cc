...
class RiscAbsOp : public OpKernel {
 public:
  ...
  void do_RiscAbsOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscAbs op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscAbsOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscAbsOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscAbsOp(fuzz_ctx);
