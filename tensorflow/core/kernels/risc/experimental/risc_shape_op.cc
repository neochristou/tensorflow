...
class RiscShapeOp : public OpKernel {
 public:
  ...
  void do_RiscShapeOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscShape op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscShapeOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscShapeOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscShapeOp(fuzz_ctx);
