...
class RiscGatherOp : public OpKernel {
 public:
  ...
  void do_RiscGatherOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscGather op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscGatherOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscGatherOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscGatherOp(fuzz_ctx);
