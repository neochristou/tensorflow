...
class RiscCholeskyOp : public OpKernel {
 public:
  ...
  void do_RiscCholeskyOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscCholesky op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscCholeskyOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscCholeskyOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscCholeskyOp(fuzz_ctx);
          fuzzer.mut_end_time(fuzz_ctx);
        }

        tffuzzing::already_fuzzing = false;
        do_RiscCholeskyOp(ctx);
      } else {
        do_RiscCholeskyOp(ctx);
      }
