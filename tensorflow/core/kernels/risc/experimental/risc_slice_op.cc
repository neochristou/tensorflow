...
class RiscSliceOp : public OpKernel {
 public:
  ...
  void do_RiscSliceOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscSlice op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscSliceOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscSliceOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscSliceOp(fuzz_ctx);
