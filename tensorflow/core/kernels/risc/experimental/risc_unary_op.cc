...
class RiscUnaryOp : public OpKernel {
 public:
  ...
  void do_RiscUnaryOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscUnary op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscUnaryOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscUnaryOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscUnaryOp(fuzz_ctx);
