...
class RiscBinaryArithmeticOp : public OpKernel {
 public:
  ...
  void do_RiscBinaryArithmeticOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscBinaryArithmetic op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscBinaryArithmeticOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscBinaryArithmeticOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscBinaryArithmeticOp(fuzz_ctx);
          fuzzer.mut_end_time(fuzz_ctx);
        }

        tffuzzing::already_fuzzing = false;
        do_RiscBinaryArithmeticOp(ctx);
      } else {
        do_RiscBinaryArithmeticOp(ctx);
      }
