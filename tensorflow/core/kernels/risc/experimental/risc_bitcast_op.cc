...
class RiscBitcastOp : public OpKernel {
 public:
  ...
  void do_RiscBitcastOp(OpKernelContext *ctx){
    // TODO(b/171294012): Implement RiscBitcast op.
  }

void Compute(OpKernelContext* ctx) override {

    if (!tffuzzing::already_fuzzing && !tffuzzing::was_fuzzed("RiscBitcastOp")) {

        tffuzzing::already_fuzzing = true;

        tffuzzing::Fuzzer fuzzer = tffuzzing::Fuzzer("RiscBitcastOp", ctx);
        OpKernelContext *fuzz_ctx;

        while (fuzzer.has_more_mutations(true)) {
          fuzz_ctx = fuzzer.get_fuzzed_context();
          fuzzer.mut_start_time();
          do_RiscBitcastOp(fuzz_ctx);
          fuzzer.mut_end_time(fuzz_ctx);
        }

        tffuzzing::already_fuzzing = false;
        do_RiscBitcastOp(ctx);
      } else {
        do_RiscBitcastOp(ctx);
      }
