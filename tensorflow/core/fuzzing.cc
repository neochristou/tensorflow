...
#include "tensorflow/core/fuzzing.h"
...
namespace fuzzing {
    ...
    Fuzzer::Fuzzer(char *fname, tensorflow::OpKernelContext* ctx)
        : cur_fname(fname), original_ctx(ctx) {
    ...
    void Fuzzer::calculate_total_mutations() {
        ...
        total_mutations = num_args * tensor_mutations.size();
    ...
    void Fuzzer::initialize_tensor_pool(){
        ...
        for (auto &tshape : tensor_shapes) {
            ...
            tensor = new Tensor(tensor_type, tshape);
    ...
    tensorflow::OpKernelContext *Fuzzer::get_fuzzed_context() {
      ...
      for (int i = 0; i num_args; i++) {
        fuzz_vec.push_back(get_next_mut());
      }

      gtl::InlinedVector<tensorflow::TensorValue, 4> fuzz_inputs =
        gtl::InlinedVector<tensorflow::TensorValue, 4>(fuzz_vec.begin(), fuzz_vec.end());
